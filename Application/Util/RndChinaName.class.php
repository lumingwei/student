...
// 获取名字
    ...
    // 获取名字
    private function getGMing()
    {
        return $this->arrGMing[mt_rand(0,$this->numbGMing-1)];
    }
    ...
    /* 获取女名列表 */
    private function getGMingList()
    {
        $this->arrGMing=array(
            '清','乐', '秀','娟','英','华','慧','巧','美','娜','静','淑','惠','珠','翠','雅','芝','玉','萍',
            '红','娥','玲','芬','芳','燕','彩','春','菊','兰','凤','洁','梅','琳','素','莲','真','环','雪',
            '爱','妹','霞','香','月','莺','媛','艳','佳','琼','珍','贞','莉','桂','娣','叶','璐','娅','琦','晶','妍','茜','秋','珊','莎','锦','黛','倩','婷','姣','婉','娴','颖','露','瑶',
            '怡','婵','雁','蓓','仪','荷','丹','蓉','眉','琴','蕊','薇','菁','梦','岚','苑','婕','馨','瑗',
            '琰','韵','咏','卿','聪','澜','悦','冰','琬','茗','欣','飘','滢','筠','柔','竹','霄','枫','芸','菲','伊','姬','枝','丽','妮','娇',
            '灵','玛','媚','琪','晴','容','雯','萱','曼','紫','蝶','菡');

        //名总数
        $this->numbGMing = count($this->arrGMing);
    }

    // 获取名字
    public function getGName($type=0)
    {
        $name = '' ;
        switch($type)
        {
            case 1:    //2字
                $name = $this->getXing().$this->getGMing();
                break;
            case 2:    //随机2、3个字
                if(mt_rand(0,100)>50){
                    $name = $this->getXing().$this->getMing().$this->getGMing();
                }else{
                    $name = $this->getXing().$this->getGMing();
                }
                break;
    ...
    {
        if(!empty($name)){
            if(in_array($name,$this->arrGMing)){
                return 2;
            }
        }
        return 1;
